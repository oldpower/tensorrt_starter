...
ckpt_path_yolo = r'../assets/train/weights/best.pt'
...
def test_01(pdfpath):
    # 使用示例
    pdf_reader = PDFStreamReader(pdfpath)
    # 获取所有页面
...
if __name__ == "__main__":
    ...
    saveimage_dir = "./assets/pdfimg"
    if not os.path.exists(saveimage_dir):
        os.makedirs(saveimage_dir)

    root_dir = Path("assets/03_chempdf_deepseek_True")
    pdf_files = list(root_dir.rglob("*.pdf"))
    num = len(pdf_files)
    for i,pdf_file in enumerate(pdf_files):
        print(f"\r进度: {i+1:06d}/{num} ", end="", flush=True)
        test_01(pdf_file)
